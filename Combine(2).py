import openpyxl
import os
# get list of all xlsx files in the current directory and return them as a list
def get_files():
...
try : 
    for i in range(len(get_column(x, search_row(x, 'Emp ID'))[1:])):
        sheet.cell(row=i+2, column=1).value = get_column(x, search_row(x, 'Emp ID'))[i+1]
        ...
        sheet.cell(row=i+2, column=4).value = get_column(x, search_row(x, 'Designation'))[i+1]
        count += 1
except TypeError:
    for i in range(len(get_column(x, search_row(x, 'Emp ID '))[1:])):
        sheet.cell(row=i+2, column=1).value = get_column(x, search_row(x, 'Emp ID '))[i+1]
...
try:
    for i in range(len(get_column(y, search_row(y, 'Emp ID'))[1:])):
        sheet.cell(row=i+count+2, column=1).value = get_column(y, search_row(y, 'Emp ID'))[i+1]
        ...
        sheet.cell(row=i+count+2, column=4).value = get_column(y, search_row(y, 'Designation'))[i+1]
        count += 1
except TypeError:
    for i in range(len(get_column(y, search_row(y, 'Emp ID '))[1:])):
        sheet.cell(row=i+count+2, column=1).value = get_column(y, search_row(y, 'Emp ID '))[i+1]
...
try:
    for i in range(len(get_column(z, search_row(z, 'Emp ID'))[1:])):
        sheet.cell(row=i+count+2, column=1).value = get_column(z, search_row(z, 'Emp ID'))[i+1]
        ...
        sheet.cell(row=i+count+2, column=4).value = get_column(z, search_row(z, 'Designation'))[i+1]
        count += 1
except TypeError:
    for i in range(len(get_column(z, search_row(z, 'Emp ID '))[1:])):
        sheet.cell(row=i+count+2, column=1).value = get_column(z, search_row(z, 'Emp ID '))[i+1]
